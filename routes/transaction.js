...
router.get("/", upload.none(), function (request, response) {
  ...
  let transactions = db
    .get("transactions")
    .filter({ account_id: request.query.account_id })
    .value();
...
router.delete("/", upload.none(), function (request, response) {
  ...
  if (removingTransaction.value()) {
    // если значение транзакции существует...
    ...
    // если значение транзакции не существует...
...
router.put("/", upload.none(), function (request, response) {
  ...
  if (!currentUser)
    // если текущего авторизованного пользователя нету
    ...
    if (reg.test(sum)) {
      ...
      response.json({
        success: false,
        error: "Недопустимые символы в поле Сумма",
      });
    }
  }
});
